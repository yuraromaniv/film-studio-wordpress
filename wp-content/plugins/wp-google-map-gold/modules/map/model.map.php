		...
		public function fetch($where = array()) {
			...
			if ( isset( $objects ) ) {
				foreach ( $objects as $object ) {
					...
					$replace_placeholder = array(
						'{location_title}' => '{marker_title}',
						'{location_address}' => '{marker_address}',
						'{location_message}' => '{marker_message}',
						'{location_latitude}' => '{marker_latitude}',
						'{location_longitude}' => '{marker_longitude}',
						'{marker_image}' => '{marker_icon}',
						'{category_title}' => '{marker_category}',
						);
					foreach ( $replace_placeholder as $find => $replace ) {
						$object->map_all_control['wpgmp_categorydisplayformat']	= str_replace( $find, $replace, $object->map_all_control['wpgmp_categorydisplayformat'] );
						if ( $object->map_all_control['infowindow_geotags_setting'] == '' ) {
							$object->map_all_control['infowindow_geotags_setting']	= '<div class="post_body">
